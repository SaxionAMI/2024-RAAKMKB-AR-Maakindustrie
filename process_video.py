...
class VideoProcessor:
    ...
    def _create_snapshot(self, file_keyword, snapshot_time,verbose=False):
        image_destination = self._output_destination / image_foldername
    ...
    def _create_step(self, idx_start, idx_stop, lines):
        ...
        filename = image_foldername + '/' + self._create_snapshot('begin', start_time).name
        ...
        if self._begin_keyword != self._end_keyword:
            ...
            filename = image_foldername + '/' + self._create_snapshot('end', end_time).name
    ...
    def _create_step_line(self, line):
        if self._snapshot_keyword in line['text'].lower():
            filename = image_foldername + '/' + self._create_snapshot_from_line(line, self._snapshot_keyword).name
...
def main():
    video_path = '/Users/saa/Library/CloudStorage/OneDrive-Saxion/Documents/AMI/ARKC/opnames/'
    video_file = 'VID_20240517_145132.mp4'
    video_file2 = 'VID_20240517_145250.mp4'

    out_path = '/Users/saa/Desktop/tp/arkc'

    p1withend = VideoProcessor(video_file=f'{video_path}/{video_file}', output_destination=out_path, snapshot_keyword='screenshot', title='VID1 with end')
    p1withend.create_description()

    p2withend = VideoProcessor(f'{video_path}/{video_file2}', output_destination=out_path, snapshot_keyword='screenshot', title='VID2 with end')
    p2withend.create_description()

    p1withoutend = VideoProcessor(video_file=f'{video_path}/{video_file}', output_destination=out_path, snapshot_keyword='screenshot', end_keyword='', title='VID1 without end')
    p1withoutend.create_description()

    p2withoutend = VideoProcessor(f'{video_path}/{video_file2}', output_destination=out_path, snapshot_keyword='screenshot', end_keyword='', title='VID2 without end')
